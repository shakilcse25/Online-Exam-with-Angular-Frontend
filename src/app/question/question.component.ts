...
export class QuestionComponent implements OnInit {
  ...
  checkQuestionValidity() {
    this.exam_service.questionValidity(this.examId, this.questionId).subscribe( ( res: any )=>{
      ...
      if( Number.isInteger(res.quesId) ) {
        this.router.navigate(['/question/' + this.examId + '/' + res.quesId ])
  ...
  setAllvalue() {
    ...
    let perDuration = (this.exam.perDuration != null) ? this.exam.perDuration : '00:01:00' ;
    var a = perDuration.split(':');
  ...
  updateAnswerSheet() {
    ...
    this.exam_service.updateAnswer( this.answerForm.value).subscribe( ( res: any )=>{
      ...
      console.log(res);
      if( this.nextId != -1 ) {
        this.router.navigate(['/question/' + this.examId + '/' + this.nextId ]);
      }else{
        this.router.navigate(['profile/result/'+this.examId ]);
      }
    } , (err: any) =>{
